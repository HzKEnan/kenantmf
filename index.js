...
const start = (bocchi = new Client()) => {
    console.log(color(figlet.textSync('T  M  F', 'Larry 3D'), 'cyan'))
    console.log(color('=> Bot başarıyla yüklendi! Veri tabanı:', 'yellow'), color(loader.getAllDirFiles('./database').length), color('Kütüphane:', 'yellow'), color(loader.getAllDirFiles('./lib').length), color('Fonksiyon:', 'yellow'), color(loader.getAllDirFiles('./function').length))
    console.log(color('=> Kaynak kod sürümü:', 'yellow'), color(version))
    console.log(color('=> Bug mu? Hatalar? Öneriler? Burayı ziyaret et', 'yellow'), color(bugs.url))
    console.log(color('[TMF]'), color('TMFWaBot - Artik Aktif!!', 'yellow'))
    console.log(color('[TMF]', 'cyan'), color('Tekrar hoş geldiniz, Sahibi! Umarım iyisindir~', 'magenta'))
    ...
    bocchi.onAddedToGroup(async (chat) => {
        ...
        console.log(color('[TMF]'), 'Added to a new group. Name:', color(chat.contact.name, 'yellow'), 'Total members:', color(chat.groupMetadata.participants.length, 'yellow'))
